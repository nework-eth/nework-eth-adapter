...
public final class ConvertUtils {
     ...
     * @return
     */
    public static <T extends Enum> T transformationStr2Enum(String str, Class<T> klass) throws IllegalArgumentException {
        return (T) Enum.valueOf(klass, str);
    }

    /**
     * 将long 类型的数据转化为 标准的时间戳
     */
    public static String getDateStringFromLong(Long timestamp, String... pattern) {
        return timestamp == null || timestamp == 0
                ? null : (pattern == null || pattern.length == 0
                ? new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date(timestamp))
                : new SimpleDateFormat(pattern[0]).format(new Date(timestamp)));
    }
}